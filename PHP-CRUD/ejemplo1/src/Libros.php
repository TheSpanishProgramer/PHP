...
class Libros extends Conexion
{
    ...
    {
        $u="update from libros set titulo=:t, isbn=:i, portada=:p, autor=:a where id_libro=:id"; //sobra el from en update
    ...
    public function totalReg()
    {
        ...
        try {
            ...
            die("Error al recuperar el total de libros, " . $ex->getMessage());
        }
    ...
    public function rellenarLibros($cant)
    {
        ...
        if ($this->totalReg() == 0) {
            ...
            for ($i = 0; $i < $cant; $i++) {
                ...
                $insert = "insert into libros(titulo, isbn, autor) values (:t, :i, :a)";
                $stmt = parent::$conexion->prepare($insert);
                try {
                    $stmt->execute([
                        ':t' => $titulo,
                        ':i' => $isbn,
                    ...
                    die("Error al crear los libros de pruebas, " . $ex->getMessage());
                }
            }
        }
    }
    ...
    public function recuperarPortada()
    {
        ...
        try {
            ...
            die("Error al recuperar la portada, " . $ex->getMessage());
        }
